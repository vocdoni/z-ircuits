import { bigIntToField, hexToField, strToBigInt, hash, uint8ArrayToBigInt, encrypt, prove, verify } from './utils';

import { buildMimc7 } from "circomlibjs";

(async () => {
    const wasm = "../../artifacts/ballot_proof_mimc_test.wasm";
    const pk = "../../artifacts/ballot_proof_mimc_test_pkey.zkey";
    const vk = "../../artifacts/ballot_proof_mimc_test_vkey.json";
    // encrypt fields
    ...
    const hFn = await buildMimc7();

    const inputs_hash = hFn.multiHash([
        inputs.process_id,
    ...
    inputs.inputs_hash = uint8ArrayToBigInt(inputs_hash.reverse());
